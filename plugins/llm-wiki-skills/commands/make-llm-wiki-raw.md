...
Create raw source note(s) in the LLM-Wiki vault. Use the vault path provided by the user, the current working directory if it is an LLM-Wiki vault, `LLM_WIKI_VAULT` if set, or the personal default `/Users/dabsdamoon/LLM-Wiki/LLM-Wiki` if it exists. If none resolve, ask the user for the vault path before writing. Read `AGENTS.md` when present. Write to `raw/inbox/` and `raw/assets/` only. Do not update `wiki/`, `wiki/index.md`, or `wiki/log.md`.
